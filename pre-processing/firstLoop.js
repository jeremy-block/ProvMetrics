 ...
 */
async function firstLoop(pathToInteractionFiles, cleanerModulePath) { 
  ...
  for (filename of fileNameList) {
    ...
    const totalDuration = [...jsonData].slice(-1)[0].time;
    const searchTermSimilarity =
      searchMetrics.ratioOfUniqueSearches(SearchList);
    const searchPeriodicity = searchMetrics.calculatePeriodicity(
      SearchList,
      totalDuration
    );
      console.log("🚀 ~ file: firstLoop.js:22 ~ firstLoop ~ filename:", filename)
      await searchMetrics.getSimilarityWordsProportion(SearchList)
        .then((searchSimilarity2) => {
          ...
          const totalInteraction = {
            ...
            prop_unique_searches: searchTermSimilarity,
